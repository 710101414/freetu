...
export default function Home() {
  ...
  const handleUpload = async (file = null, index = null) => {
    ...
    if (files.length === 0) { setUploading(false); return toast.error('未选择任何图片'); }
  ...
  const LinkRow = ({ label, value }) => (
    <div className="grid grid-cols-4 items-center gap-4 mb-3">
      <span className="col-span-1 text-right text-[12px] font-bold text-slate-500 uppercase tracking-tight">{label}</span>
    ...
    <main className="min-h-screen bg-slate-50 flex flex-col items-center pb-20">
      <header className="fixed top-0 w-full h-14 bg-white border-b flex items-center justify-between px-6 z-50 shadow-sm">
        ...
        <div className="flex items-center">
          {isAuthapi ? <LoginButton onClick={() => signOut({ callbackUrl: '/' })}>登出({role})</LoginButton> : <Link href="/login"><LoginButton>登录管理</LoginButton></Link>}
        </div>
      </header>
      ...
      <div className="mt-20 w-full max-w-4xl p-4">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm mb-6 flex justify-between items-center border">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">托管总量: {Total} | 您的IP: {IP}</p>
          </div>
          <div className="flex gap-2">
            <select value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)} className="border-2 border-slate-50 rounded-xl p-2 bg-slate-50 text-xs font-bold text-slate-600 outline-none">
        ...
        >
          ...
          {selectedFiles.map((f, i) => (
            <div key={i} className="w-32 h-44 bg-slate-50 rounded-2xl p-2 flex flex-col shadow-sm border relative z-20">
              ...
              <button onClick={() => handleUpload(f, i)} className="mt-auto text-blue-600 text-[10px] font-bold py-1">立即上传</button>
            </div>
          ))}
        ...
        {selectedFiles.length > 0 && (
          <button onClick={() => handleUpload()} className="w-full mt-4 bg-blue-600 text-white py-4 rounded-2xl font-black shadow-lg transition active:scale-95">确认开始批量上传</button>
        )}
        ...
        <div className="mt-10 bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100 min-h-[400px]">
          <div className="flex justify-between items-center mb-10 border-b pb-4">
            ...
            {isManageMode && <button onClick={handleDeleteBatch} className="bg-red-50 text-red-600 px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-red-600 hover:text-white transition shadow-sm">确认删除已选记录 ({selectedImageIds.length})</button>}
          </div>

          <div className="space-y-12">
            {uploadedImages.length === 0 && <div className="col-span-full text-center py-20 text-slate-200 italic">暂无历史记录</div>}
            {uploadedImages.map((img, i) => (
              <div 
                key={img.id || i} 
                className={`relative flex flex-col md:flex-row gap-8 p-6 rounded-3xl border transition-all ${selectedImageIds.includes(img.id) ? 'border-blue-500 bg-blue-50/50 ring-2 ring-blue-100' : 'border-slate-50 bg-slate-50/30'}`}
              >
                {/* 预览图 */}
                <div 
