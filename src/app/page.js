...
export default function Home() {
  ...
  const handleUpload = async (file = null, index = null) => {
    if (!isAuthapi || role !== 'admin') return toast.error('权限不足：请登录管理员账号');
    setUploading(true);
  ...
  const handleDeleteBatch = async () => {
    if (selectedImageIds.length === 0) return toast.warn("未选中记录");
    if (!confirm(`确定要彻底删除这 ${selectedImageIds.length} 张图片记录吗？`)) return;
  ...
  const LinkRow = ({ label, value }) => (
    <div className="grid grid-cols-4 items-center gap-4 mb-3">
      <span className="col-span-1 text-right text-xs font-bold text-slate-500 uppercase tracking-tighter">{label}</span>
      <div className="col-span-3">
        <input 
          readOnly 
          value={value} 
          onClick={(e) => { e.target.select(); handleCopy(value); }}
          className="w-full p-2 bg-slate-100 border border-slate-200 rounded text-[10px] font-mono text-slate-600 cursor-pointer hover:bg-white hover:border-blue-300 transition-all outline-none"
        />
      </div>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex flex-col items-center pb-20" onDragOver={(e) => e.preventDefault()}>
      <header className="fixed top-0 w-full h-14 bg-white border-b flex items-center justify-between px-6 z-50 shadow-sm">
        <span className="font-bold text-lg text-blue-600 tracking-tight">私人图床</span>
      ...
      <div className="mt-20 w-full max-w-4xl p-4">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm mb-6 border border-slate-100 flex flex-col md:flex-row justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { setIsManageMode(!isManageMode); setSelectedImageIds([]); }} className={`px-4 py-2 rounded-lg text-xs font-bold transition ${isManageMode ? 'bg-orange-500 text-white shadow-lg' : 'bg-slate-100 text-slate-500'}`}>
        ...
        >
          ...
          {selectedFiles.map((f, i) => (
            <div key={i} className="w-32 h-44 bg-slate-50 rounded-2xl p-2 flex flex-col shadow-sm border relative z-20">
              ...
              <button onClick={() => handleUpload(f, i)} className="mt-auto text-blue-600 text-[10px] font-bold py-1">上传此张</button>
            </div>
          ))}
          {selectedFiles.length === 0 && (
            <div className="w-full flex flex-col items-center justify-center text-slate-300 py-6 pointer-events-none">
              ...
              <p className="text-sm font-bold">粘贴图片 / 拖拽 / 点击上传</p>
            </div>
          )}
        ...
        {/* 结果展示区（Gifyu 垂直布局） */}
        <div className="mt-10 bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 min-h-[400px]">
          <div className="flex justify-between items-center mb-10 border-b pb-4">
            <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">嵌入代码</h2>
            {isManageMode && <button onClick={handleDeleteBatch} className="bg-red-50 text-red-600 px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-red-600 hover:text-white transition">删除已选 ({selectedImageIds.length})</button>}
          </div>

          <div className="space-y-12">
            {uploadedImages.length === 0 && <p className="text-center text-slate-200 italic py-20">暂无上传记录</p>}
            {uploadedImages.map((img, i) => (
              <div key={img.id || i} className={`relative flex flex-col md:flex-row gap-8 p-6 rounded-3xl border transition-all ${selectedImageIds.includes(img.id) ? 'border-blue-500 bg-blue-50/30 ring-2 ring-blue-100' : 'border-slate-50 bg-slate-50/30'}`}>
